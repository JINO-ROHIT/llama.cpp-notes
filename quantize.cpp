                        ...
                        int dtype)
{
    ...
    auto finp = std::ifstream(input_path, std::ios::binary);
    auto fout = std::ofstream(output_path, std::ios::binary);

    //use magic for limited scope
    {
        uint32_t magic;
    ...
    // load hparams
    {
        ...
        finp.read((char *) &hparams.n_rotation,   sizeof(hparams.n_rotation));
        ...
        fout.write((char *) &hparams.n_rotation,   sizeof(hparams.n_rotation));
        fout.write((char *) &dtype,           sizeof(hparams.f16));
    }
    ...
    // load vocab
    {
        ...
        if (n_vocab != hparams.n_vocab) {
            fprintf(stderr, "%s: invalid model file '%s' (bad vocab size %d != %d)\n",
                    __func__, input_path.c_str(), n_vocab, hparams.n_vocab);
            return false;
        }
